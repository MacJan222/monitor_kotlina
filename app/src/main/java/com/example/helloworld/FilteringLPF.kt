...
fun testFiltering() {
    val text = File("C:\\Users\\Maciej\\AndroidStudioProjects\\monitor_kotlina\\app\\src\\main\\java\\com\\example\\helloworld\\optic_data.txt").readText()
    val textList = text.split(" ")
    val valueList = textList.map { it.toInt() }

    val lpf = FilteringLPF(121, 30, 5, 1200,
        10, 1200, 7)

    for (i in valueList.indices) {
        lpf.processLPF(valueList[i])
        lpf.peakDetection()
    }
}
...
class FilteringLPF(
    ...
    fun processLPF(singleInput: Int): Float {
        ...
        val lpfList = MutableList(this.filterLength) { 0.toFloat() }
        for (i in lpfList.indices) { lpfList[i] = this.unfilteredChunk[i] * this.filter[i] }
